...
/// <see cref="DiscoverableTaxonomySet"/>.
/// </summary>
public class XbrlParser : IXbrlParser
{
    ...
    public XbrlParser(ILoggerFactory? loggerFactory)
    {
        ...
        _logger = _loggerFactory.CreateLogger<DiscoverableTaxonomySet>();
    }

    /// <summary>
    /// Parse XBRL documents starting from the specified entry point URI and using the provided loader to fetch documents.
    /// The resulting <see cref="DiscoverableTaxonomySet"/> contains discovered elements, contexts, units, facts and links.
    /// </summary>
    ...
    /// <returns>A task that represents the asynchronous parse operation. The task result is the populated <see cref="DiscoverableTaxonomySet"/>.</returns>
    public virtual async Task<DiscoverableTaxonomySet> ParseAsync(Uri entryPointUri, Func<Uri, Task<XDocument>> loader)
    {
    ...
    protected async Task<DiscoverableTaxonomySet> LoadDtsAsync(Uri entryPointUri, Func<Uri, Task<XDocument>> loader)
    {
        ...
        var dts = new DiscoverableTaxonomySet() { DocumentTree = tree };
        return dts;
    }

    private protected virtual (XbrlSchemaParser SchemaParser, XbrlInstanceParser InstanceParser, XbrlLinkbaseParser LinkbaseParser) CreateParsers(DiscoverableTaxonomySet dts)
    {
    ...
    void ResolveLabels(DiscoverableTaxonomySet dts)
    {
        ...
        {
            Dictionary<Concept, List<Label>> tmp = new();
    ...
    void ResolveReferences(DiscoverableTaxonomySet dts)
    {
        var linkRt = dts.RoleTypes.FirstOrDefault(rt => rt.RoleURI == XbrlNamespaces.DefaultLinkRole);
        if (linkRt != null && dts.ReferenceLinkTrees.ContainsKey(linkRt))
        {
            Dictionary<Concept, List<Reference>> tmp = new();
    ...
    void ResolveGenericLinks(DiscoverableTaxonomySet dts)
    {
